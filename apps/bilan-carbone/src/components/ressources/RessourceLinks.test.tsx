...
describe('RessourceLinks', () => {
  ...
  it('renders download buttons for links with downloadKey only', () => {
    ...
    expect(downloadButtons[0]).toHaveAttribute('data-download-key', 'SCW_CUT_METHOD_KEY')
    expect(downloadButtons[1]).toHaveAttribute('data-download-key', 'SCW_RESILIO_METHOD_KEY')
  ...
  it('opens document URL in a new tab when download succeeds', async () => {
    jest.mocked(documentsModule.getDocumentUrl).mockResolvedValue({
      success: true,
      data: 'https://storage.example.com/method.pdf',
    })

    render(
      <RessourceLinks
        title="countMethods"
        links={[{ title: 'countMethodLink', downloadKey: 'SCW_CUT_METHOD_KEY' }]}
      />,
    )
    ...
    await waitFor(() => {
      expect(documentsModule.getDocumentUrl).toHaveBeenCalledWith('SCW_CUT_METHOD_KEY')
      expect(openSpy).toHaveBeenCalledWith('https://storage.example.com/method.pdf', '_blank', 'noopener,noreferrer')
    })
  })
