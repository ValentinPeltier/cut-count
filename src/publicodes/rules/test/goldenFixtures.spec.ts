...
const FIXTURES_ROOT = path.resolve(fileURLToPath(import.meta.url), '../../../../tests/fixtures/count')
...
describe('Count golden fixtures (shared with bilan-carbone)', () => {
  ...
  test.each(['empty', 'minimal', 'rich', 'rich-all-posts'] as const)(
    'fixture %s matches expected post values',
    (name) => {
  ...
  test('bilan equals sum of post rules on minimal fixture', () => {
    ...
    const postSum = POST_RULES.reduce((acc, rule) => acc + (localEngine.evaluate(rule).nodeValue as number), 0)
